...
In short: UI → Backend → MongoDB → Backend → UI (yes, it actually works — not a tutorial illusion)
...
Frontend
HTML, CSS, JavaScript (frontend currently runs separately outside Docker)
...
🟢 Current Status
✅ Backend fully Dockerized ✅ MongoDB containerized with persistent storage ✅ API connected to database ✅ Data successfully saved and fetched from MongoDB ⚠️ Frontend not yet containerized (planned)
...
🧑‍💻 Author
Built by Mayur Joshi (With patience, debugging stamina, and way too many Docker rebuilds)
